...
- **Languages** - 8 languages (English, German, French, Turkish, Spanish, Italian, Brazilian Portuguese, Dutch), selectable on first boot or anytime from the menu. Only English is built into the firmware itself as a permanent fallback; the other 7 (plus a copy of English, for consistency) are downloaded once to the SD card the first time the device has WiFi, then loaded from there - this keeps roughly 184KB (about 9 percentage points) of flash free for other features. Switching languages loads the new one from the SD card immediately, no restart needed. If the SD card is missing, a download fails, or a language file goes missing/corrupt, the device quietly falls back to English rather than showing broken text - and re-downloads a language automatically only when its translated text has actually changed in a later firmware update, not on every single update.
