...
Every visible aircraft also gets a small triangular chevron just above or below its marker showing whether it's currently climbing or descending (derived from the same vertical rate already used for the Climb/Descent value in the detail panel) - nothing is drawn for level flight or when no vertical rate is reported, keeping the radar uncluttered.
...
**Color themes & effects:** under **Menu → System → "Radar Display"**, the **"Colors"** button switches between five color themes - Green, Amber, Blue, Red, or Purple - applied system-wide (menus, buttons, borders, text); aircraft altitude colors and alert/status colors keep their own fixed meaning regardless of theme. Six independent, combinable extras live on the same screen: a **CRT-Phosphor** glow that fades every aircraft marker color in and out as the sweep passes it; a **Radar Pulse** animation - an expanding ring from the center on every fresh data update; **Classic Radar** mode - a comet-tail sweep, extra grid spokes, a sonar-ping ring and a brief burst of signal noise whenever a new aircraft first appears; a subtle, always-on horizontal **scanline overlay** for a classic CRT-radar look; an animated **"Show Weather"** effect (see below); and a **World Map** dotted background beneath the radar circle for geographic orientation, on by default and switchable off if you find it distracting. A "?" help button next to each explains it directly on the device. A short, simulated old-radar-system startup sequence ("Initializing transponder receiver...", etc.) plays on every boot before the usual splash screen, and a **"Mode"** button in the radar screen header jumps straight to this **Radar Display** menu.

> **Note:** the UI is designed for the panel's normal (non-inverted) display mode. The separate **"Invert"** toggle (**Menu → System → Display**) flips every displayed color to its opposite, so a selected color theme will understandably no longer look like its name (e.g. selecting "Blue" can appear orange/amber) - this is expected behavior of the invert toggle, not a color-selection bug. Turn "Invert" off to see the color themes as intended.
...
- **Previously seen** - how many times the aircraft has been logged on earlier days and the date and time it was last seen, looked up from the logbook files in the background so opening the panel is never delayed.
- **Seen today** - "NEW TODAY" for a plane's first sighting of the current calendar day, "SEEN Nx TODAY" if it's already been seen multiple times today and is currently continuously visible, or "RETURNED AFTER 2h 14m" if it just reappeared after being gone for a while - resets every day (local time), independent of the persisted logbook stats above.
- **Country** - the aircraft's country of registration, derived from its ICAO 24-bit hex address (covers ~78 common countries/regions worldwide) and shown right next to the aircraft type; left out entirely if the hex code doesn't fall into a known range rather than guessing.
...
- **Flight phase** - a best-effort classification (TAKEOFF, CLIMB, CRUISE, DESCENT, APPROACH, LANDING, or LOW PASS) derived purely from altitude, vertical rate, and the existing approach detection above - shown right next to the climb/descent rate (e.g. "Climbs +850ft/min  Phase: TAKEOFF"). Left out entirely rather than guessed if the data doesn't clearly point to one phase.
- **ADS-B data quality** - shows "DATA: GOOD" or "DATA: PARTIAL" next to the altitude line depending on how many of the five core fields (position, speed, heading, altitude, callsign) the aircraft is actually transmitting - a quick way to tell a solid track from a spotty one.
...
- **Type Watchlist** - a third, similarly-structured list (**Menu → Flight Options → "Type Watch"**) stores up to 5 aircraft type codes (e.g. `A380`, `B747`, `C17`) instead of callsigns or squawk codes; any matching aircraft triggers the exact same alert (cyan ring, LED, web sound/badge, ntfy push) - handy for catching a rare heavy or a specific military type whenever it shows up, without knowing its callsign in advance.
- **Route Watchlist** - a fourth list (**Menu → Flight Options → "Route Watch"**) stores up to 5 routes by departure and/or destination airport (ICAO code, e.g. `EDDS`, `LWSK`) - either field can be left blank to match any route from/to just the other airport. Triggers the same alert as the other three lists. The route itself is looked up in the background (same multi-source lookup chain as the aircraft detail panel), only for aircraft currently visible on the radar, and cached per flight so it's only requested once - has its own **"Route Lookup"** switch (off by default) since it's the only watchlist type that needs a network lookup at all.
...
- **Airline filter** - up to 10 specific airlines (entered by IATA or ICAO code, e.g. `LH`/`DLH` for Lufthansa, `FR`/`RYR` for Ryanair - either format is auto-converted to ICAO internally), affecting the radar, aircraft list, and counts/logging. Two modes share the same list: the default **"Hide"** mode hides just those airlines, while **"Show only"** flips it into a whitelist, showing nothing but those airlines (including hiding aircraft with no recognizable airline). A mode switch with a short explanation sits right above the list, both on the device and on the Live Radar website's Lists page (kept in sync in both directions). Handy near an airport with high-frequency airlines you'd rather not see - or, in "Show only" mode, to focus on just a handful of carriers.
...
- **Show helicopters only (ON/OFF)** - filters everything down to helicopters only, hiding everything else. Off by default; handy for rotorcraft-specific traffic (medevac, police, sightseeing tours).
- **Show low-altitude aircraft only (ON/OFF)** - filters down to aircraft currently below the green altitude threshold. Ground vehicles are never included by this filter regardless of the separate ground-vehicle setting. Off by default.
- **Interesting Only (ON/OFF)** - shows only aircraft that already trigger at least one existing detection: a military/government flight, an emergency squawk, a hit on any of the four watchlists above, or a Heavy aircraft (ADS-B emitter category). A pure combination of existing detections, nothing new to configure. Off by default; has its own "?" info button since it bundles several criteria.
- **Empty-sky filter hint** - when the radar shows no aircraft because a filter (helicopters only, low-altitude only, interesting only, airline filter) is hiding everything, the "empty sky" message names which filter is active and is tappable, jumping straight to the Display Filters menu.

All five toggles live under **Menu → Flight Options → Display Filters**, and apply consistently everywhere aircraft are counted or listed - the radar itself, the [Aircraft List](#-aircraft-list), the Live Traffic dashboard, and the Live Radar web page/map all show exactly the same set of aircraft for a given filter combination.
...
**Optional GPS module:** a "GPS" button next to "Auto" turns reading from a connected GPS module on/off (only has an effect when "Auto" is active). With GPS enabled and a module wired up, your location follows your movement live (e.g. while driving) instead of only being estimated via IP; without a connected module, the button simply has no effect. Plug the module into the board's built-in GPS connector (4-pin JST header labeled VIN/TX/RX/GND) - wired to GPIO1, 9600 baud (NMEA protocol); confirmed working with the common, inexpensive BN-180 module. See the "?" info button on the Location Presets screen for the same details on-device. Once the module has a fix, the radar screen's info bar also shows your live position (and altitude, as soon as the module reports one) right below the altitude legend - it only appears while GPS is enabled and has a fix, otherwise the info bar looks exactly as before.

**⚠️ Important hardware note:** GPIO1 (TXD0) is the same pin used by the board's onboard CH340 USB chip (UART0). This means the GPS module and a USB connection can't be used at the same time - **unplug the GPS module before flashing over USB or using the serial monitor**, otherwise the upload/monitor will fail since the USB chip's TX line is blocked. This is a hardware limitation of the board itself, not something a setting can work around. Regular [OTA updates](#-ota-firmware-updates-over-wifi) are unaffected and work fine with the GPS module still connected, since they don't use the USB/UART0 connection at all.

**Follow-Me Mode** (**Menu → System → Display → Radar Display**, off by default): built for mobile use with a connected GPS module. When enabled, the radar centers live on your current GPS position (using the same GPS infrastructure as above, even overriding an active location preset while enabled), rotates the whole radar so "up" always points in your current direction of travel instead of north, and automatically zooms further out at higher speed so aircraft further ahead stay visible - all smoothed over several GPS fixes to avoid a jittery, jumpy rotation. Enabling it also turns GPS reading on automatically. Without an active GPS fix, the radar cleanly falls back to the normal, fixed north-up view - no error, no crash.
...
#### 📈 Session Statistics
**Menu → Flight Options → Stats & Logbook → "Session Statistics"** shows a handful of purely in-memory highlights since the last restart, independent of the flight logbook (ON/OFF) switch below: the number of distinct aircraft seen, the closest pass (distance + callsign), the highest speed and altitude recorded, and the most common aircraft type. Everything here resets on every reboot and is clearly labeled "since last restart", so it's never confused with the persisted daily/all-time numbers in the regular Statistics screen above.
...
An optional push-notification integration (**Menu → System → Tools → "ntfy.sh Push"**, off by default) sends a notification straight to your phone via the free [ntfy.sh](https://ntfy.sh) service whenever an emergency squawk or a watchlist match is detected - no account or server of your own needed, just install the free ntfy app (or use a browser) and subscribe to a topic name of your choosing. Turn it on, enter that topic (kept effectively private simply by picking a hard-to-guess name, the same way any ntfy.sh topic works), and use the "Test" button to send yourself a sample notification and confirm everything's wired up correctly.

**Flight Stories** - a separate, independently switchable "Flight Stories" checkbox on the same ntfy.sh Push screen (off by default) sends a short automatic notification whenever a military/government aircraft, a helicopter, or an unusually low-flying aircraft is spotted - only for aircraft actually visible on the radar (current range and filters apply). It shares the same single-notification send slot as the emergency/watchlist alert above, which always takes priority if both happen at once, and suppresses repeat messages for the same aircraft for 10 minutes even if it stays in range longer.

**Quiet Hours** - an optional toggle on the same ntfy.sh Push screen (off by default) with a freely adjustable start/end hour (full hours, 0-23, correctly handling a window that spans midnight, e.g. 22 to 7) pauses every push type - emergency/watchlist alerts, Flight Stories, and approach alerts - during that window. Only the push notification to your phone is affected; the on-device display and LED alarms keep working exactly as usual. The "Test" button still works during Quiet Hours, so you can always confirm your setup is correct.
...
- **Screen timeout with slider** (**Menu → System → "Screen Timeout"**) - sets how long until the display turns off after a period of no touches, from 1 to 15 minutes or "Never", using a drag slider instead of tapping through values one minute at a time. Defaults to 5 minutes on a fresh install (existing devices keep whatever they already had set).
...
- **System status** (**Menu → System → Tools → "System Status"**) - a second diagnostic screen showing live WiFi signal strength (RSSI), free heap memory, the largest contiguous free memory block (with a "?" info button explaining why that number matters separately from plain free heap), how long the most recent ADS-B fetch took in milliseconds, and the current Performance Auto-Tuning level (see below). Updates live while the screen is open.
- **Performance Auto-Tuning** (**Menu → System → Display → Radar Display**, on by default) - automatically and gradually reduces render load in up to 3 steps (first the weather visual effect, then the sweep animation's comet-tail, then aircraft silhouette detail) whenever sluggishness or tight memory is detected - based on sweep-timing, free heap, and the largest contiguous free memory block, the same figures shown on the System Status screen. Automatically steps back down again once things recover, with some hysteresis so it doesn't flip back and forth. Purely visual - never affects the actual flight data. Can be turned off entirely if you'd rather always have full detail.
- **Menu structure** - the Flight Options menu is grouped into category buttons: Lists (Aircraft List, Watchlist, Squawk Watchlist, Type Watch, Route Watch), Stats & Logbook, LED Alerts, Display Filters (airline filter, ground vehicles, helicopters only, low-altitude only, interesting only), and a direct Location Presets button. The System menu has its own Display/Tools categories.
- **Quick header shortcuts** - tapping the WiFi signal bars in the header jumps to WiFi settings, tapping the small clock jumps to screen-timeout settings, and a small eye icon between "Menu" and the WiFi bars jumps to the Display Filters menu - the entire header is interactive. A large, centered QR code linking to this project's GitHub page is available under **Menu → System → Tools → "About"**. The same QR code also appears at the bottom of the two firmware-update screens (installing/installed), generated live rather than stored as an image.
...
Tapping the icon opens a small info popup explaining that the shown weather reflects the currently active location. The same popup shows the raw METAR text for the nearest airport (from the free [aviationweather.gov](https://aviationweather.gov) data API), fetched in the same background cycle as the icon weather, today's date, today's sunrise and sunset time for the active location (calculated entirely on-device, no extra network request), and a short forecast (temperature and conditions) 3 hours ahead. Sunrise/sunset shows a neutral placeholder instead of the date/time if the device doesn't yet have GPS/location data or a synchronized clock. Wind direction and speed, parsed straight out of the same METAR text, are shown as a compass direction and speed (following the Metric/Imperial units setting), with calm and variable wind handled specially; the web page additionally draws a small rotated arrow. The popup scrolls automatically if all of this doesn't fit on one screen.
