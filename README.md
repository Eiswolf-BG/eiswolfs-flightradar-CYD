...
- **Intelligent proximity alert** – an alternative alert mode (**Menu → Flight Options → LED Alerts → "Alert mode"**) that replaces the single-radius alert with three staggered distance zones (yellow/orange/red), only triggering when an aircraft genuinely moves into a tighter zone (not while it stays there or moves back out), each zone blinking progressively faster; an altitude filter avoids false alarms from high-flying long-haul traffic passing overhead
