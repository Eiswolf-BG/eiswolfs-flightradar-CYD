...
- **Radar Display** – choose between Green, Amber, or Blue for the radar screen (sweep line, panel border/text, buttons) under **Menu → System → "Radar Display"**, plus two independent, combinable extras: a CRT-Phosphor glow effect that fades *every* aircraft marker color (not just low-altitude/green ones) in and out as the sweep passes them, and a Radar Pulse animation - an expanding ring from the radar center on every fresh data update
